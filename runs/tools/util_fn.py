...
from llib.models.utils.transform import get_affine_transform
...
def process_cond(cond, c, s, image_size, pixel_std=200.0):
    trans = get_affine_transform(c, s, pixel_std, 0, image_size)
    
    cond = np.concatenate((cond, np.ones((cond.shape[0], 1))), axis=1)
    cond = cond @ trans.T
