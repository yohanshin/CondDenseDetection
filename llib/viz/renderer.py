...
class Renderer():
    ...
    def create_lights(self, location=[[0.0, 0.0, -10.0]]):
        ...
        location_hom = torch.cat((location, torch.ones_like(location[..., -1:])), dim=-1)
        extrinsic = torch.cat((self.cameras.R, self.cameras.T.unsqueeze(-1)), dim=-1)
        location = (extrinsic @ location_hom.mT).squeeze(-1)
