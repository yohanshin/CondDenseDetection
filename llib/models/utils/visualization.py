...
def check_results(images, outputs, iteration, exp_name):
    ...
    writer = imageio.get_writer(f"experiments/{exp_name}/visualization/results_{iteration}.mp4", fps=1)
...
def compare_results(images, preds, targets, iteration, path):
    ...
    writer = imageio.get_writer(f"{path}/results_{iteration}.mp4", fps=1)
...
def compare_results_denseldmks2d(images, pred_vals, targets, masks, iteration, path, normalize_plus_min_one=False):
    ...
    images = images.detach().cpu()
    images = images * torch.tensor(std).view(1, 3, 1, 1) + torch.tensor(mean).view(1, 3, 1, 1)
    images = (images * 255).numpy().astype(np.uint8).transpose(0, 2, 3, 1)[..., ::-1]   # Why converting back to BGR?
    ...
    writer = imageio.get_writer(video_fn, fps=1)
...
def compare_results_denseldmks2d(images, pred_vals, targets, iteration, path, normalize_plus_min_one=True):
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]
